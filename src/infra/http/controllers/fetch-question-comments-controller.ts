...
export class FetchQuestionCommentsController {
  ...
  async handle(
    ...
    const result = await this.fetchQuestionCommentss.execute({ page, questionId })

    if (result.isLeft()) throw new BadGatewayException()

    const questionComments = result.value.questionComments.map(CommentPresenter.toHTTP)
